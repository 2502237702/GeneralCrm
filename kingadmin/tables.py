...
class TableHandler(object):
    def __init__(self, request, model_class, admin_class, query_sets, order_res):
        ...
        self.colored_fields = getattr(admin_class, 'colored_fields') if hasattr(admin_class, 'colored_fields') else {}

        self.dynamic_fk = getattr(admin_class, 'dynamic_fk') if hasattr(admin_class, 'dynamic_fk') else None
        self.dynamic_list_display = getattr(admin_class, 'dynamic_list_display') if hasattr(admin_class,
                                                                                            'dynamic_list_display') else ()
        self.dynamic_choice_fields = getattr(admin_class, 'dynamic_choice_fields') if hasattr(admin_class,
                                                                                              'dynamic_choice_fields') else ()
