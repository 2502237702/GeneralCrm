...
from GeneralCrm import settings
...
def acc_login(request):
    ...
    cache.set(random_filename, random_code,6000)
    ...
    if request.method == "POST":
        ...
        if cache.get(_verify_code_key).upper() == _verify_code.upper():
...
def enrollment(request, customer_id):
    ...
    model_form = king_admin_forms.create_form(
        models.Enrollment,
        fields,
    ...
    if request.method == "POST":
        ...
        exist_enrollment_objs = models.Enrollment.objects.filter(
        ...
        if exist_enrollment_objs:
            ...
            if exist_enrollment_objs.filter(contract_agreed=True):
                ...
                if enroll_obj.contract_approved or request.POST.get('contract_approved') == "on":
                    ...
                    if enroll_obj.paymentrecord_set.select_related().count() > 0:
