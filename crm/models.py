    ...
    def clean_consultant(self):
        ...
        if self.instance.id == None :
            ...
            raise forms.ValidationError(
                ('Invalid value: %(value)s 课程顾问不允许被修改,shoud be %(old_value)s'),
                code='invalid',
...
class Enrollment(models.Model):
    ...
    why_us = models.TextField("报名理由", max_length=1024, default=None, blank=True, null=True)
    your_expectation = models.TextField("具体期望", max_length=1024, blank=True, null=True)
    contract_agreed = models.BooleanField("我已认真阅读完协议并同意全部内容")
    contract_approved = models.BooleanField("审批通过", help_text=u"在审阅无误后勾选此项,合同即生效")
...
class UserProfile(auth.AbstractBaseUser, auth.PermissionsMixin):
    ...
    roles = models.ManyToManyField('Role', blank=True, verbose_name="权限角色")
    ...
    class Meta:
        ...
        permissions = (
            ...
            ('crm_app_tables', '可以访问 所有列表的内容'),
            ('crm_table_add_view', '可以访问所有列表添加页面'),
            ('crm_table_add_add', '可以添加所有列表'),
            ('crm_table_del_view', '可以访问所有列表删除页'),
            ('crm_table_del_del', '可以删除所有列表'),
            ('crm_personal_password_reset_view', '访问管理员修改密码页'),
            ('crm_personal_password_reset', '管理员修改密码'),
            ('crm_password_reset_view', '访问个人修改密码页'),
            ('crm_password_reset', '个人修改密码'),
